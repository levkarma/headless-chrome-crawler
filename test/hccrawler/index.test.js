  ...
  describe("HCCrawler.launch", () => {
    ...
    describe("when the server is running", () => {
      ...
      describe("when the crawler is launched with necessary options", () => {
        ...
        test("skips crawling when the requested url has been previously scraped", async () => {
          this.server.setContent("/", `<a href="${PREFIX}/one">One</a>`);
          this.server.setContent("/one", `<a href="${PREFIX}/">Index</a>`);
          let requestskipped = 0;
          this.crawler.on("requestskipped", () => {
            requestskipped += 1;
          });
