...
class Crawler {
  ...
  async _scrape() {
    ...
    const result = await this._page.evaluate(this._options.evaluatePage);
    debug("_scrape() return");
    return result;
  }
  ...
  async _screenshot() {
    ...
    if (!this._options.screenshot) {
      debug("_screenshot() return");
      return null;
    }
    const result = await this._page.screenshot(this._options.screenshot);
    debug("_screenshot() return");
    return;
  }
  ...
  async _collectLinks(baseUrl) {
    ...
    debug("_collectLinks() exposeFunction() done");
    ...
    debug("_collectLinks() links", links);
    debug("_collectLinks() return");
    return uniq(links);
  }
