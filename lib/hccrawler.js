const debug = require("debug")("hccrawler");
...
class HCCrawler extends EventEmitter {
  ...
  static async launch(options) {
    debug("picked launch options", pick(options, LAUNCH_OPTIONS));
  ...
  constructor(browser, options) {
    ...
    this._queue.on("pull", (_options, depth, previousUrl) => {
      debug("pull event received, starting request");
      return this._startRequest(_options, depth, previousUrl);
    });
    this._browser.on(
      "disconnected",
  ...
  async _startRequest(options, depth, previousUrl) {
    ...
    if (skip) {
      debug("_startRequest(): skipping request");
    ...
    if (!allowed) {
      debug("_startRequest(): not allowed");
    ...
    debug("_startRequest(): _request() finished");
  ...
  async _skipRequest(options) {
    ...
    // checkRequested checks if previously requested
  ...
  async _request(options, depth, previousUrl, retryCount = 0) {
    debug("_request(): start");
    if (this.pastGlobalTimeout()) return;
  ...
  async _followLinks(urls, options, depth) {
    ...
    await Promise.all(
      map(urls, async url => {
        ...
        if (skip) {
          this.emit(HCCrawler.Events.RequestSkipped);
          return;
        }
  ...
  pastGlobalTimeout() {
    if (
      this._options.globalTimeout > 0 &&
      Date.now() - this._options.globalTimeout > this._startTime
    ) {
      this.emit(HCCrawler.Events.GlobalTimeout, this._options.globalTimeout);
      return true;
    }
  }
